...
( function() {
	...
	CKEDITOR.plugins.add( 'imageresponsive', {
		...
		init: function( editor ) {
			...
			// when we'll rename imageresponsive to image.
			var config = editor.config,
				lang = editor.lang.imageresponsive,
				image = widgetDef( editor );

			// Since filebrowser plugin discovers config properties by dialog (plugin?)
			// names (sic!), this hack will be necessary as long as imageresponsive is not named
			// Image. And since imageresponsive will never be Image, for sure some filebrowser logic
			// got to be refined.
			config.filebrowserimageresponsiveBrowseUrl = config.filebrowserImageBrowseUrl;
			config.filebrowserimageresponsiveUploadUrl = config.filebrowserImageUploadUrl;
			...
			CKEDITOR.dialog.add( 'imageresponsive', this.path + 'dialogs/imageresponsive.js' );
		},
	...
	// 		└──────┴───────────────────────────────┴─────────────────────────────┘
	//
	// Non-captioned widget (config.imageresponsive_alignClasses defined)
	// 		┌──────┬───────────────────────────────┬─────────────────────────────┐
	// 		│Align │Internal form                  │Data                         │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	// 		│none  │<wrapper>                      │<img />                      │
	...
	// 		└──────┴────────────────────────────────────────┴────────────────────────────────────────┘
	//
	// Captioned widget (config.imageresponsive_alignClasses defined)
	// 		┌──────┬────────────────────────────────────────┬────────────────────────────────────────┐
	// 		│Align │Internal form                           │Data                                    │
	// 		├──────┼────────────────────────────────────────┼────────────────────────────────────────┤
	// 		│none  │<wrapper>                               │<figure />                              │
	...
	function widgetDef( editor ) {
		var alignClasses = editor.config.imageresponsive_alignClasses,
			captionedClass = editor.config.imageresponsive_captionedClass;
		...
		return {
			...
			init: function() {
				var helpers = CKEDITOR.plugins.imageresponsive,
			...
			// Overrides default method to handle internal mutability of imageresponsive.
			...
			getClasses: ( function() {
				...
				return function() {
					...
					// Neither config.imageresponsive_captionedClass nor config.imageresponsive_alignClasses
	...
	CKEDITOR.plugins.imageresponsive = {
		stateShifter: function( editor ) {
			// Tag name used for centering non-captioned widgets.
			var doc = editor.document,
				alignClasses = editor.config.imageresponsive_alignClasses,
				captionedClass = editor.config.imageresponsive_captionedClass,
				...
				hasCaption:	function( shift, oldValue, newValue ) {
					...
					if ( newValue ) {
						...
						var figure = CKEDITOR.dom.element.createFromHtml( templateBlock.output( {
							...
							captionPlaceholder: editor.lang.imageresponsive.captionPlaceholder
						} ), doc );
	...
	// <figure class="{config.imageresponsive_captionedClass}"> elements.
	//
	...
	function downcastWidgetElement( editor ) {
		...
		return function( el ) {
			...
			// inline styles or classes (imageresponsive_alignClasses).
	...
	function centerWrapperChecker( editor ) {
		var captionedClass = editor.config.imageresponsive_captionedClass,
			alignClasses = editor.config.imageresponsive_alignClasses,
		...
		return function( el ) {
			...
			// Centering wrapper got to be... centering. If imageresponsive_alignClasses are defined,
	...
	function setupResizer( widget ) {
		...
		resizer.setAttribute( 'title', editor.lang.imageresponsive.resizer );
	...
	// on configurations like config#imageresponsive_alignClasses or
	// config#imageresponsive_captionedClass.
	//
	// @param {CKEDITOR.editor}
	// @returns {Object}
	function getWidgetAllowedContent( editor ) {
		var alignClasses = editor.config.imageresponsive_alignClasses,
			rules = {
				...
				figure: {
					classes: '!' + editor.config.imageresponsive_captionedClass
				},
				figcaption: true
			};
 ...
 *		CKEDITOR.config.imageresponsive_captionedClass = 'captionedImage';
 *
 * @cfg {String} [imageresponsive_captionedClass='image']
 * @member CKEDITOR.config
 */
CKEDITOR.config.imageresponsive_captionedClass = 'image';
 ...
 *
 *		config.imageresponsive_alignClasses = [ 'align-left', 'align-center', 'align-right' ];
 *
 ...
 *
 * @since 4.4
 * @cfg {String[]} [imageresponsive_alignClasses=null]
 * @member CKEDITOR.config
 */
